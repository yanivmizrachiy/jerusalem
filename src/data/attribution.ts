 ...
 *
 * ראיות ייחוס מותרות (אותם שמות ואותו מספור בדיוק כמו RULES 24.1.2
 * ובהערות הקבוצה שב-author-assignments.ts — אין לקסיקון מקביל):
 * 1. `publisher-domain` — המשאב עצמו הוא פרסום רשמי של הגוף ומוגש
 *    מהדומיין הרשמי שלו; חל על ארגון בלבד. עצם אירוח קישור חיצוני אינו מספיק.
 * 2. `owner-verified` — מסירה מפורשת של בעל הפרויקט לגבי היוצר/הגוף היוצר.
 * 3. `explicit-credit` — שם היוצר מופיע במפורש בכותרת/תיאור/קרדיט של המשאב.
 * 4. `district-designated` — מקור שסעיפי 9.1 ו-9.4–9.6 ב-RULES מגדירים
 *    במפורש כסביבה של המחוז; לשלושת האתרים (misparim, zaviyot, tzirim)
 *    נמדד ב-09/08/2026 גם קרדיט עצמי חי בתוך הסביבה עצמה.
 *
