...
test('download action is truthful about same-origin versus external files', async () => {
  ...
  // must only emit the attribute for a root-relative local URL. Protocol-relative
  // URLs (`//cdn.example/...`) are cross-origin candidates and must stay in open mode.
  expect(actions).toContain(
    "const localDownload = Boolean(downloadUrl?.startsWith('/') && !downloadUrl.startsWith('//'))",
  );
  ...
  expect(actions).not.toContain("const localDownload = Boolean(downloadUrl?.startsWith('/'))");
