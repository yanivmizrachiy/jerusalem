...
**ניווט מתחת לספר (05/08/2026):** סרגל דף-קודם/דף-הבא נשאר צמוד לספר, ומתחתיו `nav.book-links` — שורת קישורים פנימיים לאזורים אחרים בפרויקט (חטיבה עליונה, חוזר מפמ״ר, לוח, הודעות, פיתוח מקצועי, ראשי). מוגדרת ב-`siteLinks` בתוך `Booklet.astro`, מוסתרת ב-`is-full` (`display:none`), ולכן אינה נספרת ב-`pageSize()` ואינה משנה את גובה הבמה. מופיעה גם בעמודי ה-reader.
