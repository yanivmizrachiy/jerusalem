...
- שער חטיבת הביניים (10/08): המסך הראשון הוא רצועה רחבה — כותרת + טבלת הקישורים מימין וההטמעה הקומפקטית של ההמחשות (`HamchashotEmbed compact`) משמאל, כולה בתוך המסך הראשון; מתחתיו שלושה שלישים לכיתות ז׳/ח׳/ט׳, ולאחריהם CTA אמיתי ל־`/chativat-beynayim/mivchanim/`. אין עוד הטמעת המחשות בתחתית השער; בעמודי המבוא של השכבות ההטמעה הגבוהה נשארת.
