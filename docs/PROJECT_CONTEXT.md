...
**עמוד משאב** — `ResourceSplit.astro` דרך `reader/[grade]/[item].astro`: הכתובות הקנוניות לא השתנו. `.res-view` (ימין) מול `.res-panel` (שמאל) בגריד `1.05fr / 0.95fr`; בנייד עמודה אחת וההטמעה ראשונה. **פס גלילה אחד (06/08/2026):** במסך ≥901px סקריפט מודד את הגובה הפנוי ומציב `--fit` על `.res-split` — כולל חיסור של כל מה שמתחת לחלוקה (ריפוד ופס התחתית), אחרת הפוטר דוחף את העמוד ומחזיר פס גלילה שני. הצדדים ב-`block-size:100%`, ה-iframe ממלא את מסגרתו, והפאנל גולל בתוך עצמו (`overflow-y:auto`). פריט `maf` חוזר ל-`MafmarRange fill` — עכשיו יש לו הורה בגובה קבוע. בנייד הכול חוזר לזרימה טבעית. ההטמעה נטענת ב-IntersectionObserver עם `.res-skel` ורשת ביטחון של 8ש׳; `navigator.pdfViewerEnabled === false` מחליף אותה בכרטיס פתיחה (`[hidden]{display:none!important}` — הכרטיס לעולם לא מתחת להטמעה חיה). קודם/הבא נגזרים מ-`itemNeighbours()` לפי סדר הפרקים. פריט `maf` מוגש כ-`MafmarRange tall` (לא `fill` — `fill` דורש הורה בגובה קבוע).
