...
`src/data/choveret.ts` מגדיר שכבות, פרקים ופריטים; ממנו נגזרים שלושת המסכים, בלי רשימה מקבילה בשום עמוד. **החוברת המדפדפת בוטלה ב-05/08/2026** (הוראת יניב, RULES 3.26, 3.29): `Booklet.astro` נמחק, התלות `page-flip` הוסרה, ואין להחזירם.
...
**עמוד משאב** — `ResourceSplit.astro` דרך `reader/[grade]/[item].astro`: הכתובות הקנוניות לא השתנו. `.res-view` (sticky, ימין) מול `.res-panel` (שמאל) בגריד `1.05fr / 0.95fr`; בנייד עמודה אחת וההטמעה ראשונה. ההטמעה נטענת ב-IntersectionObserver עם `.res-skel` ורשת ביטחון של 8ש׳; `navigator.pdfViewerEnabled === false` מחליף אותה בכרטיס פתיחה (`[hidden]{display:none!important}` — הכרטיס לעולם לא מתחת להטמעה חיה). קודם/הבא נגזרים מ-`itemNeighbours()` לפי סדר הפרקים. פריט `maf` מוגש כ-`MafmarRange tall` (לא `fill` — `fill` דורש הורה בגובה קבוע).
...
- **כלי ניהול אישיים אינם חיים בריפו הזה** (הוראת יניב, 05/08/2026): הסטודיו
  ...
  לכל. הסטודיו ממשיך לפרסם לאתר כרגיל (GitHub API עם משתני סביבה ב-Vercel); רק
  מיקום הקוד השתנה. **קובץ היעד שלו — `Booklet.astro` — נמחק ב-05/08/2026 עם ביטול
  החוברת (RULES 3.29, 4.14); היעד החדש טעון הכרעת יניב.** בתיקייה
