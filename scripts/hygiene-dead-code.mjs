...
const candidates = [];
for (const sourceFile of program.getSourceFiles().filter(isSrcTs)) {
  for (const statement of sourceFile.statements) {
    ...
    let references = 0;
    for (const other of program.getSourceFiles().filter(isSrcTs)) {
      const visit = (node) => {
...
if (!FIX) {
  console.error('\nRun `node scripts/hygiene-dead-code.mjs --fix` to remove only these symbol-proven declarations.');
  process.exit(1);
}
